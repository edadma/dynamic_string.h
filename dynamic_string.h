...
DS_DEF ds_string ds_append_char(ds_string str, char c);
...
DS_DEF int ds_sb_append_char(ds_stringbuilder *sb, char c);
...
static ds_data* ds_data_create(size_t length) {
    // Single allocation: struct + string data + null terminator
    ds_data *data = (ds_data*)DS_MALLOC(sizeof(ds_data) + length + 1);
    if (!data) return NULL;
...
DS_DEF ds_string ds_append(ds_string str, const char *text) {
    ...
    size_t new_length = str.data->length + text_len;
    ds_string result;
...
DS_DEF ds_string ds_insert(ds_string str, size_t index, const char *text) {
    ...
    memcpy(result.data->array + index + text_len,
           str.data->array + index,
           str.data->length - index);
...
DS_DEF ds_string ds_concat(ds_string a, ds_string b) {
    if (!a.data && !b.data) return DS_NULL_STRING;
    if (!a.data) return ds_retain(b);
    if (!b.data) return ds_retain(a);

    size_t new_length = a.data->length + b.data->length;
    ds_string result;
    result.data = ds_data_create(new_length);
...
DS_DEF size_t ds_length(ds_string str) {
    return str.data ? str.data->length : 0;
}

DS_DEF const char* ds_cstr(ds_string str) {
    return (str.data) ? str.data->array : "";
}
...
DS_DEF int ds_find(ds_string str, const char *needle) {
    if (!str.data || !needle) return -1;

    char *found = strstr(str.data->array, needle);
    return found ? (int)(found - str.data->array) : -1;
}

DS_DEF int ds_starts_with(ds_string str, const char *prefix) {
    if (!str.data || !prefix) return 0;

    size_t prefix_len = strlen(prefix);
    if (prefix_len > str.data->length) return 0;

    return memcmp(str.data->array, prefix, prefix_len) == 0;
}

DS_DEF int ds_ends_with(ds_string str, const char *suffix) {
    if (!str.data || !suffix) return 0;

    size_t suffix_len = strlen(suffix);
    if (suffix_len > str.data->length) return 0;

    return memcmp(str.data->array + str.data->length - suffix_len, suffix, suffix_len) == 0;
}

DS_DEF size_t ds_ref_count(ds_string str) {
    return str.data ? str.data->ref_count : 0;
}

DS_DEF int ds_is_shared(ds_string str) {
    return str.data && str.data->ref_count > 1;
}
...
static int ds_sb_ensure_capacity(ds_stringbuilder *sb, size_t required_capacity) {
    ...
    ds_data *new_data = (ds_data*)DS_REALLOC(sb->data, sizeof(ds_data) + new_capacity);
    if (!new_data) {
...
static int ds_sb_ensure_unique(ds_stringbuilder *sb) {
    ...
    size_t current_length = sb->data->length;
    size_t new_capacity = sb->capacity;

    // Create new data structure
    ds_data *new_data = (ds_data*)DS_MALLOC(sizeof(ds_data) + new_capacity);
    if (!new_data) {
        return 0; // Failed to allocate
    }
...
DS_DEF ds_stringbuilder ds_sb_create(void) {
    return ds_sb_create_with_capacity(DS_SB_INITIAL_CAPACITY);
}

DS_DEF ds_stringbuilder ds_sb_create_with_capacity(size_t capacity) {
    ds_stringbuilder sb;

    if (capacity == 0) capacity = DS_SB_INITIAL_CAPACITY;
...
DS_DEF int ds_sb_append(ds_stringbuilder *sb, const char *text) {
    ...
    if (!ds_sb_ensure_capacity(sb, sb->data->length + text_len + 1)) return 0;
...
DS_DEF int ds_sb_append_char(ds_stringbuilder *sb, char c) {
    ...
    // Ensure we have a unique copy and enough capacity
    if (!ds_sb_ensure_unique(sb)) return 0;
    if (!ds_sb_ensure_capacity(sb, sb->data->length + 2)) return 0;

    // Append the character
    sb->data->array[sb->data->length] = c;
    sb->data->length++;
    sb->data->array[sb->data->length] = '\0';
...
DS_DEF int ds_sb_append_string(ds_stringbuilder *sb, ds_string str) {
    if (!sb || !str.data) return 0;

    // Ensure we have a unique copy and enough capacity
    if (!ds_sb_ensure_unique(sb)) return 0;
    if (!ds_sb_ensure_capacity(sb, sb->data->length + str.data->length + 1)) return 0;
...
DS_DEF int ds_sb_insert(ds_stringbuilder *sb, size_t index, const char *text) {
    if (!sb || !text || !sb->data || index > sb->data->length) return 0;
    ...
    memmove(sb->data->array + index + text_len,
            sb->data->array + index,
            sb->data->length - index + 1);
...
DS_DEF void ds_sb_clear(ds_stringbuilder *sb) {
    ...
    if (!ds_sb_ensure_unique(sb)) return;
...
DS_DEF ds_string ds_sb_to_string(ds_stringbuilder *sb) {
    ...
    // THE MAGIC: Shrink to exact size and prepare for sharing
...
DS_DEF size_t ds_sb_length(const ds_stringbuilder *sb) {
    return (sb && sb->data) ? sb->data->length : 0;
}
...
DS_DEF const char* ds_sb_cstr(const ds_stringbuilder *sb) {
    return (sb && sb->data) ? sb->data->array : "";
}

// Legacy compatibility
DS_DEF void ds_free(ds_string *str) {
    ds_release(str);
}
